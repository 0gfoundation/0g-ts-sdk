...
- `tests/encryption.vectors.test.ts` — hardcoded hex test vectors
  generated from the Go `core` package. Each vector fixes a known key /
  nonce / recipient privkey / ephemeral privkey / plaintext and asserts
  the ts-sdk produces byte-identical header + ciphertext. Catches any
  silent incompatibility drift from library upgrades.
...
1. **`tests/encryption.test.ts`** — unit tests of the primitives. Covers
   header parse/serialize for v1 and v2, `cryptAt` at offset 0 and
   non-zero offsets, ECIES derive-key roundtrip, wrong-privkey-
   differs-from-right-privkey, parse rejects unsupported version / too-
   short headers, `resolveDecryptionKey` errors cleanly when the
   required material is missing for the header version.

2. **`tests/encryption.vectors.test.ts`** — fixed vectors generated from
   the Go `core` package. Three vectors minimum:
   - v1 known key + known nonce + known plaintext → expected ciphertext
     and header bytes.
   - v2 known recipient privkey + known ephemeral privkey + known
     nonce + known plaintext → expected header bytes and ciphertext.
     (Test-only; production uses fresh random ephemerals.)
   - Offset decryption: encrypt 100 bytes at offset 0, decrypt bytes
     32..100 at offset 32, assert they match bytes 32..100 of the
     straightforward full decrypt.

   Vectors live in `tests/encryption.vectors.ts` as hex constants.
   Generated by running a short ad-hoc program against the Go `core`
   package (commit 6d39443) that prints hex to stdout; the output is
   pasted into the vectors file. The generator script itself is not
   committed — the vectors are the durable artifact.

3. **`tests/encrypted_file.test.ts`** — integration at the file-wrapper
